...
const createDeviceStmt = db.prepare(
  "INSERT INTO devices (id, owner_name, owner_email, owner_phone, reward, registered_at) VALUES (?, ?, ?, ?, ?, datetime('now'))",
);
const saveReportStmt = db.prepare(
  'INSERT INTO reports (device_id, finder_name, finder_contact, finder_photo_path, latitude, longitude, ip, email_sent) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
);
const updateEmailSentStmt = db.prepare(
  'UPDATE reports SET email_sent = ? WHERE id = ?',
);
const listReportsStmt = db.prepare(
  'SELECT * FROM reports WHERE device_id = ? ORDER BY created_at DESC',
);
